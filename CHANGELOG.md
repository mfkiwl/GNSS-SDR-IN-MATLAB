...
- `docs/M4_summary.md`：**M4 阶段总结报告**——交付物清单、验证结果汇总
  （Synthetic 218 m / 硬件 133 m）、9 条关键技术结论与踩坑、复现步骤、
  现状与下一步（bias-T 真实卫星定位）

## [0.15.0] - 2026-08-10

### Changed

- `docs/HANDOFF.md`：**第四次交接文档全面重写**——M1→M4 全部成果、
  30 个 .m 资产清单、硬件功率标定（单星 -65 dB / 多星 -60 dB 两档）、
  36 条关键技术结论与踩坑（新增 M4 亚毫秒比特边界/多周期歧义/ECEF 位移/
  struct-cell 陷阱等）、完整复现步骤、已知风险与下一步（bias-T 真实卫星）
